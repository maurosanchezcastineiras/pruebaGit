...
const FormularioRangoTemperaturaEditar = () => {
    const { id } = useParams();
    ...
    useEffect(() => {
        const buscaRangos = async () => {
            try {
                const resultado = await axios.get(`http://localhost:4000/rangosTemperatura/${id}`);
                setFormulario({
    ...
    const editarRangos = async (e) => {
        e.preventDefault();
        try {
            await axios.put(`http://localhost:4000/rangosTemperatura/${id}`, formulario);
            navega("/");
        } catch (error) {
            console.log(error);
        }  
...
export default FormularioRangoTemperaturaEditar
